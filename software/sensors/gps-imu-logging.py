...
# Define GPS Serial Port (Modify this based on your setup)
GPS_PORT = "/dev/ttyS0"  # Change to "/dev/ttyUSB0" if using a USB GPS
BAUD_RATE = 9600  # Standard GPS baud rate
gps_serial = serial.Serial(GPS_PORT, BAUD_RATE, timeout=1)  # Open serial connection to GPS

# Define JSON file for logging data
LOG_FILE = "gps_imu_log.json"
...
def log_data(entry):
    """
    Saves sensor data into a JSON file. Appends data if the file already exists.
    """
    if os.path.exists(LOG_FILE):  # Check if the JSON file exists
    ...
    data.append(entry)  # Append new entry to the list

    # Write back to the JSON file (indent=4 for readability)
...
def main():
    ...
    while True:
        try:
            ...
            log_entry = {
                "timestamp": timestamp,
                "imu": imu_data,  # Full IMU data (accel, gyro, temp)
                "gps": gps_data if gps_data else "No GPS Fix",
                "heading_estimate": estimated_heading
            }
